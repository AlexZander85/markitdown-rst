...
**Converts 13+ formats · SIMD-accelerated (AVX-512/AVX2/NEON) · Dozens of times faster than Python · 20–50 MB RAM · Single binary**
...
|--------|-------|--------|--------|----------|
| 📕 **PDF** | `.pdf` | Markdown, HTML, DOCX, JSON | `pdf-extract` + `lopdf` | Page-by-page extraction, heading detection |
| 📘 **DOCX** | `.docx` | Markdown, HTML, DOCX, JSON | ZIP + `quick-xml` | Bold, italic, headings, tables, breaks |
| 📗 **XLSX** | `.xlsx` | Markdown tables, HTML, DOCX, CSV, JSON | `umya-spreadsheet` | Multi-sheet, empty sheet handling |
| 📙 **PPTX** | `.pptx` | Per-slide Markdown, HTML, DOCX, JSON | ZIP + `quick-xml` | Text from slides and shapes |
| 🌐 **HTML** | `.html`, `.htm` | Markdown, HTML, DOCX, JSON | `html2text` | `<title>` extraction |
...
| 🖼️ **Images** | `.jpg`, `.png`, `.tiff`, `.bmp`, `.gif`, `.webp` | Markdown (OCR), HTML, DOCX, JSON | **Tesseract** | 3 languages: EN / RU / ZH |
...
|--------|------|-------|--------|-------------|
| 📕 **PDF** | `.pdf` | Markdown, HTML, DOCX, JSON | `pdf-extract` + `lopdf` | Постраничное извлечение, определение заголовков |
| 📘 **DOCX** | `.docx` | Markdown, HTML, DOCX, JSON | ZIP + `quick-xml` | Жирный, курсив, заголовки, таблицы, разрывы |
| 📗 **XLSX** | `.xlsx` | Markdown-таблицы, HTML, DOCX, CSV, JSON | `umya-spreadsheet` | Мультилистовые таблицы, обработка пустых листов |
| 📙 **PPTX** | `.pptx` | Markdown по слайдам, HTML, DOCX, JSON | ZIP + `quick-xml` | Извлечение текста из слайдов и фигур |
| 🌐 **HTML** | `.html`, `.htm` | Markdown, HTML, DOCX, JSON | `html2text` | Извлечение заголовка `<title>` |
...
| 🖼️ **Изображения** | `.jpg`, `.png`, `.tiff`, `.bmp`, `.gif`, `.webp` | Markdown (OCR), HTML, DOCX, JSON | **Tesseract** | 3 языка: EN / RU / ZH |
...
9. **SIMD-ускоренный парсинг** — `simd-json` (2–5× JSON), `regex` (SIMD DFA), `bytecount` (SIMD подсчёт), `blake3` (SIMD хеш), `memchr` (SIMD поиск) — всё с автоопределением CPU при запуске, не нужны отдельные сборки
...
| 格式 | 输入 | 输出 | 引擎 | 特性 |
|------|------|------|------|------|
| 📕 **PDF** | `.pdf` | Markdown、HTML、DOCX、JSON | pdf-extract + lopdf | 逐页提取、标题检测 |
| 📘 **DOCX** | `.docx` | Markdown、HTML、DOCX、JSON | ZIP + quick-xml | 粗体、斜体、标题、表格、换行 |
| 📗 **XLSX** | `.xlsx` | Markdown 表格、HTML、DOCX、CSV、JSON | umya-spreadsheet | 多工作表、空工作表处理 |
| 📙 **PPTX** | `.pptx` | 按幻灯片 Markdown、HTML、DOCX、JSON | ZIP + quick-xml | 从幻灯片和形状提取文本 |
| 🌐 **HTML** | `.html`、`.htm` | Markdown、HTML、DOCX、JSON | html2text | 提取 `<title>` |
| 🗂️ **XML** | `.xml` | Markdown、HTML、DOCX、JSON | quick-xml | 深度感知格式化，按级别标题 |
...
| 🖼️ **图像** | `.jpg`、`.png`、`.tiff`、`.bmp`、`.gif`、`.webp` | Markdown（OCR）、HTML、DOCX、JSON | **Tesseract** | 3 种语言：EN / RU / ZH |
| 🗜️ **ZIP** | `.zip` | 索引、统计、Markdown、HTML、DOCX、JSON | zip crate | 内容分析、文本提取（最大 50 KB） |
...
mdrust-cli cpu-info                  # 查看 CPU SIMD 功能和加速信息
mdrust-cli ocr-check                 # 检查 Tesseract 和 tessdata
mdrust-cli convert doc.pdf -f html   # 转换为 HTML
mdrust-cli convert doc.pdf -f docx   # 转换为 Word
...
- 📤 `-f html` / `-f docx`——导出为 HTML5 或 Word 格式
- 🧮 `cpu-info` 命令——检查 CPU SIMD 能力（AVX-512、AVX2、SSE4.2、NEON）
